<template lang="pug">
  Form(:label-width="80")
    ...
    Form-item(label="设备图片")
      Input(v-model="form.imageUrl", @on-change="e => change('imageUrl', e.target.value)")
    Form-item(label="备注信息")
      Input(v-model="form.remark", type="textarea", @on-change="e => change('remark', e.target.value)", :rows="2")
</template>
<script>
